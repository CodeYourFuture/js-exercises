...
var restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function(numberOfPeople) {
    var number = this.restaurants
      .filter(key => key.totalSeats > numberOfPeople)
      .map(key => key.name);
  ...
  findRestaurantServingDish: function(dishName) {
    var dishAre = this.restaurants
      .filter(key => key.menu.includes(dishName))
      .map(key => key.name);
  ...
  countNumberOfRestaurantsInArea: function(area) {
    var addressCenter = this.restaurants
      .filter(key => key.address.area === area)
      .map(key => key.name);

    return addressCenter.length;
    // Complete here
  }
};
