...
var restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function(numberOfPeople) {
    ...
    restaurants.map(
      item =>
        item.totalSeats - item.numberOfCustomers > numberOfPeople
          ? resname.push(restaurants.name)
          : ""
    );
    return resname;
    // Complete here
  ...
  findRestaurantServingDish: function(dishName) {
    ...
    restaurants.map(item =>
      item.menu.filter(
        element => (element === dishName ? rname.push(item.name) : "")
      )
    );
    return rname;
  },
  ...
  countNumberOfRestaurantsInArea: function(area) {
    var groupOne = [];
    restaurants.map(
      item => (item.address.area === area ? groupOne.push(item.name) : "")
    );
    return groupOne.length;
  }
};
/*
