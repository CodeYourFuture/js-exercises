...
var restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function(numberOfPeople) {
    return this.restaurants
      .filter(rest => rest.totalSeats - rest.numberOfCustomers > numberOfPeople)
      .map(rest => rest.name);
  },
  ...
  findRestaurantServingDish: function(dishName) {
    // Complete here
    return this.restaurants
      .filter(res => res.menu.includes(dishName))
      .map(rest => rest.name);
  },
  countNumberOfRestaurantsInArea: function(area) {
    // Complete here
    return this.restaurants.filter(rest => rest.address.area.includes(area))
      .length;
  }
};
