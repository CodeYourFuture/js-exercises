...
var restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function(numberOfPeople) {
    var arr1 = restaurants
      .filter(function(oggetto) {
        return numberOfPeople <= oggetto.totalSeats - oggetto.numberOfCustomers;
      })
      .map(oggetto => oggetto.name);
    return arr1;

    /* var prova = restaurants
      .map(function(oggetto) {
        if (numberOfPeople <= oggetto.totalSeats - oggetto.numberOfCustomers) {
          return oggetto.name;
        }
      })
  ...
  findRestaurantServingDish: function(dishName) {
    var arr = restaurants
      .filter(oggetto => oggetto.menu.includes(dishName))
      .map(oggetto => oggetto.name);
    return arr;

    /*Define a method findRestaurantServingDish which takes a dish name in parameter and returns
all the restaurant names serving this dish.*/
  },
  countNumberOfRestaurantsInArea: function(area) {
    var arr = restaurants.filter(oggetto => oggetto.address.area == area)
      .length;
    return arr;

    /*Define a method countNumberOfRestaurantsInArea which takes an area of Glasgow in parameter (centre, west),
and returns the number of restaurants in this area. */
  }
};
