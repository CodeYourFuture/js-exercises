...
var restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function(numberOfPeople) {
    var findAvailRests = restaurants.filter(function(rest) {
      return rest.totalSeats - rest.numberOfCustomers >= numberOfPeople;
    });
    return findAvailRests.map(getRestaurantName);
    // Complete here
  },
  findRestaurantServingDish: function(dishName) {
    var findDish = restaurants.filter(function(rest) {
      return rest.menu.includes("salad");
    });
    return findDish.map(getRestaurantName);
    // Complete here
  },
  countNumberOfRestaurantsInArea: function(area) {
    var restsInCityArea = restaurants.filter(function(rest) {
      return rest.address.area === area;
    });
    return restsInCityArea.length;
    // Complete here
  }
};
