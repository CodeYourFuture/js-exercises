...
var restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function(numberOfPeople) {
    return restaurants
      .filter(
        numSeatAvail =>
          numSeatAvail.totalSeats - numSeatAvail.numberOfCustomers >
          numberOfPeople
      )
      .map(numSeatAvail => numSeatAvail.name);
  },

  findRestaurantServingDish: function(dishName) {
    return restaurants
      .filter(newDish => newDish.menu.includes(dishName))
      .map(newDish => newDish.name);
  },

  countNumberOfRestaurantsInArea: function(area) {
    return restaurants.filter(newArea => newArea.address.area === area).length;
  }
};
