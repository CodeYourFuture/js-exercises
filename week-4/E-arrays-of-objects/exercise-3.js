...
*/
const getName = restaurant => restaurant.name;
var restaurantFinderApplication = {
  applicationName: "Restaurant Finder",
  applicationVersion: "1.0",
  restaurants: restaurants,
  findAvailableRestaurants: function(numberOfPeople) {
    const availableRestaourants = restaurants.filter(
      restaurant =>
        numberOfPeople <= restaurant.totalSeats - restaurant.numberOfCustomers
    );
    return availableRestaourants.map(getName); // Complete here
  },
  findRestaurantServingDish: function(dishName) {
    const servingRestaurant = restaurants.filter(restaurant =>
      restaurant.menu.includes(dishName)
    );
    return servingRestaurant.map(getName);
    // Complete here
  },
  countNumberOfRestaurantsInArea: function(area) {
    const restaurantsByArea = restaurants.filter(
      restaurant => restaurant.address.area === area
    );
    return restaurantsByArea.length;
    // Complete here
  }
};
