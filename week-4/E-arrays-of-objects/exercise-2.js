...
var destinationNamesWithin500Kms = travelDestinations
  .map(function(elemento) {
    if (elemento.distanceKms < 500) {
      return elemento.destinationName;
    }
  })
...
var destinationNameReachableByFerry = travelDestinations
  .map(function(oggetto) {
    if (oggetto.transportations.includes("ferry")) {
      return oggetto.destinationName;
    }
  })
  .filter(function(elem) {
    if (elem) {
...
travelDestinations.forEach(function(oggetto) {
  if (oggetto.transportations.includes("train") && oggetto.distanceKms > 300) {
    destinationNamesMoreThan300KmsAwayByTrain.push(oggetto.destinationName);
  }
});
