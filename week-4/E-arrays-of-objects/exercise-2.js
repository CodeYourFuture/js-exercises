...
var destinationNamesWithin500Kms = travelDestinations
  .filter(des2 => des2["distanceKms"] < 500)
  .map(des => des.destinationName); // Complete here
var destinationNameReachableByFerry = travelDestinations
  .filter(tran => tran.transportations.includes("ferry"))
  .map(tran => tran.destinationName); // Complete here
var destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(key => {
    if (key["distanceKms"] > 300 && key["transportations"].includes("train")) {
      return key;
    }
  })
  .map(trans => trans.destinationName); // Complete here (PRINT THE RESULT IN THE CONSOLE USING FOREACH)
