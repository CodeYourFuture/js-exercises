...
var destinationNamesWithin500Kms = travelDestinations
  .filter(newDestination => newDestination.distanceKms <= 500)
  .map(newDestination => newDestination.destinationName); // Complete here

var destinationNameReachableByFerry = travelDestinations
  //.filter(reachByFerry => reachByFerry.transportations === "ferry")
  .filter(reachByFerry => reachByFerry.transportations.includes("ferry"))
  .map(reachByFerry => reachByFerry.destinationName); // Complete here

var destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(a => a.distanceKms > 300 && a.transportations.includes("train"))
