...
function reachable(destination) {
  if (destination.distanceKm < 500);
  {
    console.log(destination.name);
  }
}
var destinationNamesWithin500Kms = travelDestinations.map(reachable);
// Complete here
function reachableFerry(destination) {
  if (destination.transportations.includes("ferry"));
  return destination.name;
}
var destinationNameReachableByFerry = travelDestinations.filter(reachableFerry); // Complete here

function reachableTrain(destination) {
  if (
    destination.transportations.includes("train") &&
    destination.distanceKm < 350
  );
  return destination.name;
}
