...
var destinationNamesWithin500Kms = travelDestinations // Complete here

  .filter(milley)
  .map(milley1);
function milley(destin) {
  return destin.distanceKms < 500;
}
function milley1(destin) {
  return destin.destinationName;
}

//console.log(destinationNamesWithin500Kms);

var destinationNameReachableByFerry = travelDestinations // Complete here
  .filter(milley2)
  .map(milley3);
function milley2(nam) {
  return nam.transportations.includes("ferry");
}
function milley3(nam) {
  return nam.destinationName;
}
...
var destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(milley4)
  .map(milley5);
function milley4(nam) {
  return nam.distanceKms > 300 && nam.transportations.includes("train");
}
function milley5(nam) {
  return nam.destinationName;
}
