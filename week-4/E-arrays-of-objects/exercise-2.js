...
*/
const getName = destination => destination.destinationName;
const whitin500KM = destination => destination.distanceKms < 500;
const byFerry = destination => destination.transportations.includes("ferry");
const moreThan300KM = destination => destination.distanceKms > 300;
const byTrain = destination => destination.transportations.includes("train");
...
var destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(moreThan300KM)
  .filter(byTrain)
  .map(getName); // Complete here (PRINT THE RESULT IN THE CONSOLE USING FOREACH)
