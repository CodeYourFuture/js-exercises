...
let destinationNamesWithin500Kms = travelDestinations
  .filter((p) => p.distanceKms < 500)
  .map((p) => p.destinationName); // Complete here

let destinationNameReachableByFerry = travelDestinations
  .filter((p) => p.transportations.includes("ferry"))
  .map((p) => p.destinationName); // Complete here

let destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter((p) => p.distanceKms > 300 && p.transportations.includes("train"))
  .map((p) => p.destinationName); // Complete here (PRINT THE RESULT IN THE CONSOLE USING FOREACH)
/*
