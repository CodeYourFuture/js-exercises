...
let destinationNamesWithin500Kms = travelDestinations
  .filter((getDistance) => getDistance.distanceKms < 500)
  .map((getDistanceNames) => getDistanceNames.destinationName); // Complete here

let destinationNameReachableByFerry = travelDestinations
  .filter((getTransport) => getTransport.transportations.includes("ferry"))
  .map((getDistanceNamesBy) => getDistanceNamesBy.destinationName); // Complete here
let destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(
    (getDestinationNames) =>
      getDestinationNames.distanceKms > 300 &&
      getDestinationNames.transportations.includes("train")
  )
  .map((getDestinationNamesB) => getDestinationNamesB.destinationName);
