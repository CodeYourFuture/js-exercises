...
let destinationNamesWithin500Kms = travelDestinations.filter(destination => destination.distanceKms < 500).map(distination => distination.destinationName)
...
let destinationNameReachableByFerry = travelDestinations.filter(destination => destination.transportations.includes("ferry")).map(distination => distination.destinationName)
