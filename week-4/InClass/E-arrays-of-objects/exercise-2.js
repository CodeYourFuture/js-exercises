...
let destinationNameReachableByFerry = travelDestinations
  .filter((city) => city.transportations.includes("ferry"))
  .map(getCityName);
console.log(destinationNameReachableByFerry);
// Complete here (PRINT THE RESULT IN THE CONSOLE USING FOREACH) complete here
// 3  ///
function isFurtherThan300KmByTrain(city) {
  return city.distanceKms > 300 && city.transportations.includes("train");
}
let destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(isFurtherThan300KmByTrain)
  .map(getCityName);
