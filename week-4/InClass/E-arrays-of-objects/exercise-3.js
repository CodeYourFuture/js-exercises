...
let restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function (numberOfPeople) {
    return restaurants
      .filter(
        (rest) => rest.totalSeats - rest.numberOfCustomers > numberOfPeople
      )
      .map((key) => key.name);
  },
  ...
  countNumberOfRestaurantsInArea: function (area) {
    let restInArea = restaurants.filter((rest) =>
      rest.address.area.includes(area)
    );
    return restInArea.length;
  },
};
