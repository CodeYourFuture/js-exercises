...
let restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function (numberOfPeople) {
    let numberOfAllAvailableRestaurantNames = restaurants
      .filter(
        (numOfSeatsAvailable) =>
          numOfSeatsAvailable.totalSeats -
            numOfSeatsAvailable.numberOfCustomers >=
          numberOfPeople
      )
      .map((getRestaurantNames) => getRestaurantNames.name);

    return numberOfAllAvailableRestaurantNames;

    // console.log(numberAllOfPeople);
    // Complete here
  },
  findRestaurantServingDish: function (dishName) {
    let findRestaurantNamesServingSalad = restaurants
      .filter((findSaladDish) => findSaladDish.menu.includes(dishName))
      .map((getRestaurantNames) => getRestaurantNames.name);
    return findRestaurantNamesServingSalad;
    // Complete here
  },
  countNumberOfRestaurantsInArea: function (area) {
    let bringTheLengthOfRestaurant = restaurants.filter((filterCenter) =>
      filterCenter.address.area.includes(area)
    ).length;
    // Complete here
    return bringTheLengthOfRestaurant;
  },
};
