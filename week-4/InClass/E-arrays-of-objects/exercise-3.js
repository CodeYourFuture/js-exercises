...
let restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function (numberOfPeople) {
    ...
    let result = restaurants
      .filter((i) => i.totalSeats - i.numberOfCustomers > numberOfPeople)
      .map((j) => j.name);
    return result;
  },
  findRestaurantServingDish: function (dishName) {
    // Complete here
    return restaurants
      .filter((i) => i.menu.includes(dishName))
      .map((j) => j.name);
  },
  countNumberOfRestaurantsInArea: function (a) {
    // Complete here
    let abc = restaurants
      .filter((i) => i.address.area.includes(a))
      .map((j) => j.name);
    let result = abc.length;
    return result;
  },
};
