...
let restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function (numberOfPeople) {
    return restaurants
      .filter((p) => p.totalSeats - p.numberOfCustomers >= numberOfPeople)
      .map((p) => p.name);
    // Complete here
  },
  findRestaurantServingDish: function (dishName) {
    return restaurants
      .filter((p) => p.menu.includes(dishName))
      .map((p) => p.name); // Complete here
  },
  countNumberOfRestaurantsInArea: function (area) {
    return restaurants.filter((p) => p.address.area.includes(area)).length; // Complete here
  },
};
