...
let restaurantFinderApplication = {
  ...
  findAvailableRestaurants: function (numberOfPeople) {
    return (numberOfPeople = this.restaurants
      .filter((seats) => seats.totalSeats - seats.numberOfCustomers > 5)
      .map((names) => names.name));
  },
  findRestaurantServingDish: function (dishName) {
    return (dishName = this.restaurants
      .filter((dish) => dish.menu.includes("salad"))
      .map((name) => name.name));
  },
  countNumberOfRestaurantsInArea: function (area) {
    return (area = restaurants.filter(
      (location) => location.address.area === "center"
    ).length);
  },
};
