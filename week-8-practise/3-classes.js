 ...
 *  - Return and compare the price of the two housese value of the house should increase by 80%
...
console.log(
  `There is ${House.feetIntoMetersSquar(950)} meter squar in 950 foot squar`
);

/*When we declare a method as static it's not called on the instances(example:brownHouse, blueHouse )
instead they are called on the class it self in other words we don't need to create an instance of 
that class to access  the method we can access it directly use class name and the dot notation 
for example: House.feetIntoMetersSquar(950), if we tried to access a static method using instance
it will give us an error for example: brownHouse.feetIntoMetersSquar(950)  will generate this error 
"TypeError: brownHouse.feetIntoMetersSquar is not a function"
*/
