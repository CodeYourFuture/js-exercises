...
function replace(arr, valueToReplace, newValue) {
  var indexOfValueToReplace = arr.indexOf(valueToReplace);
  var leftValues = arr.slice(0, indexOfValueToReplace);
  var rightValues = arr.slice(indexOfValueToReplace + 1);
  var leftValueAndNuewValue = leftValues.concat(newValue);
  var finalValue = leftValueAndNuewValue.concat(rightValues);
  return finalValue;
}
