...
let myEmailInput = document.querySelectorAll(
  'div[class*="form-group"]>input[id*=exampleInputEmail1'
);
let myNameInput = document.querySelectorAll(
  'div[class*="form-group"]>input[id*="example-text-input"]'
);
let myDescribeInput = document.querySelectorAll(
  'div[class*="form-group"]>textarea[id*="exampleTextarea"]'
);
...
function checkValid() {
  for (let i = 0; i < MyArrayOfMyRegisterInput.length; i++) {
    console.log(MyArrayOfMyRegisterInput[i]);
    if (
       MyArrayOfMyRegisterInput[i].querySelector("input") &&
      (!MyArrayOfMyRegisterInput[i].querySelector("input").value.length > 0) ||
        (MyArrayOfMyRegisterInput[i].querySelector("input").type === "email")  &&
        (!MyArrayOfMyRegisterInput[i].querySelector("input").value.includes("@")) 
    ) { 
        MyArrayOfMyRegisterInput[i].querySelector("input").style.backgroundColor = 'red';
      }
      else{
      alert("thank you for filling out the form");
      MyArrayOfMyRegisterInput[i].querySelector("input").value = "";
      console.log(MyArrayOfMyRegisterInput[i].querySelector("input").value);
    }
    //    else if(MyArrayOfMyRegisterInput[i].querySelector("textarea").value){

    //    }
    // let describe = MyArrayOfMyRegisterInput[i].querySelector("textarea").value;
