...
function insertPeopleData(arrayOfPeople) {
  ...
  for (let i = 0; i < arrayOfPeople.length; i++) {
    ...
    h1Tag.textContent = arrayOfPeople[i].name;
    content.appendChild(h1Tag);

    var h2Tag = document.createElement("h2");
    h2Tag.textContent = arrayOfPeople[i].job;
...
function insertBooks(books) {
  ...
  for (let i = 0; i < books.length; i++) {
    ...
    bookInfo.innerText = books[i].title + " By " + books[i].author;
    uList.appendChild(bookList);
