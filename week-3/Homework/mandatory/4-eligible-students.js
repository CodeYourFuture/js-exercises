...
function getEligibleStudents(studentsArr) {
  let elegible = studentsArr.filter((student) => student[1] >= 8);
  const studentNames = elegible.map((names) => names[0]);
  return studentNames;
}
...
function getEligibleStudents2(studentNames) {
  let findA = studentNames.filter(
    (names) => names[0].charAt(0) === "A" || names[0].charAt(0) === "a"
  );
  return findA.map((x) => x[0]);
}
