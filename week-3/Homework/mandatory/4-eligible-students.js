...
function getEligibleStudents(alphaStudentGroup) {
  let allStudents = alphaStudentGroup.filter((i) => i[1] >= 8);
  let nameofStudents = allStudents.map((i) => i[0]);
...
function getEligibleStudents2(array) {
  let nameofStudents = array.map((i) => i[0].toUpperCase());
  let selectedStudent = nameofStudents.filter((i) => i[0].startsWith("A"));
  let result = selectedStudent;
  let final = result.map((i) => i.toLowerCase());
...
test(
  "eligibleStudents2 function works",
  arraysEqual(getEligibleStudents2(alphaStudentGroup), ["Ahmed", "Adam"])
);

test(
  "eligibleStudents2 function without eligible candidates",
  arraysEqual(getEligibleStudents2(deltaStudentGroup), ["amy"])
);

test(
  "eligibleStudents2 function with no candidates",
