...
function getEligibleStudents(ages) {
  let bigger = ages.filter((newArray) => newArray[1] >= 8).map((x) => x[0]);
  return bigger;
}
...
function getEligibleStudents2(names) {
  let findWithA = names.filter(
    (newArray) => newArray[0].startsWith("A") || newArray[0].startsWith("a")
  );

  return findWithA.map((x) => x[0]);
}
/* ======= TESTS - DO NOT MODIFY ===== */
...
console.log(getEligibleStudents2(deltaStudentGroup));
