...
function getEligibleStudents(students) {
  let chooseStudent = students.filter((array) => array[1] >= 8);

  return chooseStudent.map((names) => names[0]);
}
...
function getEligibleStudents2(students) {
  let findStudents = students.filter(
    (members) => members[0].charAt(0) === "A" || members[0].charAt(0) === "a"
  );
  return findStudents.map((names) => names[0]);
}
