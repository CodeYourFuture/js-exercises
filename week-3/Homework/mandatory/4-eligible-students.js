...
function getEligibleStudents(num) {
  let pupils = num.filter((x) => x[1] >= 8).map((p) => p[0]);
  return pupils;
}
...
function getEligibleStudents2(startA) {
  let students = startA.filter(
    (p) => p[0].startsWith("A") || p[0].startsWith("a")
  );
  return students.map((x) => x[0]);
}
...
console.log(getEligibleStudents(alphaStudentGroup));
