...
// Example 1
let pets = twoDimensionalArray[0][1]; // this reads the first embedded array
...
function getEligibleStudents(arr) {
  return arr
    .filter((subArray, index) => subArray[1] >= 8)
    .map((subArray, index) => subArray[0]);
}
...
function getEligibleStudents2(arr) {
  return arr
    .filter((subArray, index) => subArray[0][0].toUpperCase() === "A")
    .map((subArray, index) => subArray[0]);
}
