...
function getEligibleStudents(arr) {
  let newArr = arr.filter((attendance) => attendance[1] >= 8);

  return newArr.map((name) => name[0]);
}
/*
...
function getEligibleStudents2(names) {
  let result = names.filter(
    (firstLetter) =>
      firstLetter[[0]].charAt(0) === "A" || firstLetter[0].charAt(0) === "a"
  );
  return result.map((name) => name[0]);
}
