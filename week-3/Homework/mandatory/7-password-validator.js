...
function validatePasswords(passwords) {
  const validCombinations = /(?=.{5,})(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[!#$%.])/;
  const passwordValidationResult = passwords.map((password, index) => {
    return (
      validCombinations.test(password) && passwords.indexOf(password) === index
    );
  });
