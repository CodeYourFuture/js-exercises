...
function validatePasswords(password) {
  let newArr = password.map(password => uppercase.test(password) && numbers.test(password) &&
   lowercase.test(password)&& password.length >= 5 && nonAlphanumeric.test(password))
  return newArr;
}
console.log(validatePasswords(["Se%5", "TE.TJTU", "384#HsHF", "dvyyeyy!5", "tryT3729"]))
