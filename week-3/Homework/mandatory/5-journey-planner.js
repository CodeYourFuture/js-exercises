...
function getTransportModes(array) {
  let foundTRasnport = array.slice(1, 3);
  return foundTRasnport;
}
...
*/
function isAccessibleByTransportMode(transport, modes) {
  if (transport.includes(modes)) {
    return true;
  } else {
    return false;
  }
...
function journeyPlanner(locations, transportMode) {
  let found = locations.filter((p) => p.includes(transportMode));
  return found.map(getLocationName);
