window.addEventListener('load',()=>{//executes the event when the page loads 
  ...
  if(navigator.geolocation){
    navigator.geolocation.getCurrentPosition(position =>{// getCurrentPosition is function that gets the latitude and longitude of position
      ...
      const api =`${proxy}https://api.darksky.net/forecast/fd9d9c6418c23d94745b836767721ad1/${lat},${long}`;// Api url injected with latitude and logitude
      fetch(api)// fetches the api
      .then((response) => {// converts a data into json from the api
      ...
      .then(data =>{
                     ...
                     let celsius = (temperature - 32) * (5 / 9);// converts Feren to Celcius
                     ...
                     temperatureSectionSpan.addEventListener("click", () => { // adds an event listener to the span
                       if (temperatureSectionSpan.textContent === "F") {// if the temperature span === F it would change to C
                         ...
                         temperatureDegree.textContent = temperature;// change the temperature degree to Farenhighi
                       }
                     });
                   });
        ...
        function setIcons(icon,iconID){// function that gets an icon  an icon id as a parameter.
